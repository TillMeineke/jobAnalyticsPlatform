...
This directory contains data loading pipelines that process job data from the bronze layer and load it into the silver and gold layers.

## 🚀 Available Pipelines

- `process_jobs.py`: Processes raw job postings from the bronze layer, performs data cleaning, and loads them into the silver layer

## 📋 Usage

```bash
# Process job data through the pipeline
python process_jobs.py
```

## 🔧 Configuration

The pipeline reads data from the `data/bronze/` directory and outputs processed data to `data/silver/` and `data/gold/` directories.
